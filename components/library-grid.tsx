    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <input
          ...
          className="h-9 w-full max-w-sm border bg-background px-3 text-sm outline-none focus-visible:border-ring"
        />
        {entities && (
          ...
          {state?.nextCursor && (
            <div className="flex justify-center">
              <button
                ...
                className="inline-flex h-9 items-center border px-4 text-sm hover:bg-accent hover:text-accent-foreground disabled:opacity-50 cursor-pointer outline-none focus-visible:border-ring"
              >
...
function LibraryCard({
  ...
  function save() {
    ...
    startTransition(async () => {
      ...
      const res = await fetch(
        `/api/entities/${encodeURIComponent(entity.slug)}`,
        {
          method: "PATCH",
  ...
  function remove() {
    if (
      !confirm(
        `Delete "${entity.displayName}"? Future renders won't auto-resolve this slug.`,
      )
    )
      return;
    startTransition(async () => {
          ...
          <div
            className={
              entity.shapePreference === "circle"
                ? "flex h-16 w-16 items-center justify-center border bg-destructive/10 text-[10px] text-destructive"
                : "flex h-16 w-16 items-center justify-center border bg-destructive/10 text-[10px] text-destructive"
            }
            ...
            <>
              <label className="block">
                ...
                <input
                  ...
                  className="h-7 w-full border bg-background px-2 text-sm outline-none focus-visible:border-ring"
                />
              </label>
              <label className="block">
                <span className="block text-sm font-medium text-muted-foreground">
                ...
                <input
                  ...
                  className="h-7 w-full border bg-background px-2 font-mono text-xs outline-none focus-visible:border-ring"
            ...
            <>
              ...
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                {entity.isMine && (
                  <>
                    <span>·</span>
                    <span className="text-[10px] text-primary">mine</span>
                  </>
                )}
              </div>
            </>
        ...
        {editing ? (
          <>
            <button
              ...
              onClick={save}
              disabled={pending}
              className="inline-flex h-7 items-center bg-primary px-3 text-xs font-medium text-primary-foreground disabled:opacity-50"
            >
              {pending ? "Saving..." : "Save"}
            </button>
            <button
              type="button"
              onClick={onDone}
              className="inline-flex h-7 items-center border px-3 text-xs hover:bg-accent hover:text-accent-foreground cursor-pointer outline-none focus-visible:border-ring"
            >
              Cancel
            </button>
            <label className="inline-flex h-7 cursor-pointer items-center border px-3 text-xs hover:bg-accent hover:text-accent-foreground outline-none focus-visible:border-ring">
              Replace logo
              <input
                type="file"
                accept="image/*"
            ...
            <button
              ...
              onClick={remove}
              disabled={pending}
              className="ml-auto inline-flex h-7 items-center border border-destructive/40 px-3 text-xs text-destructive hover:bg-destructive/10 disabled:opacity-50 cursor-pointer outline-none focus-visible:border-ring"
            >
              Delete
            </button>
          </>
          ...
          <button
            ...
            onClick={hide}
            disabled={pending}
            className="inline-flex h-7 items-center border px-3 text-xs hover:bg-accent hover:text-accent-foreground disabled:opacity-50 cursor-pointer outline-none focus-visible:border-ring"
