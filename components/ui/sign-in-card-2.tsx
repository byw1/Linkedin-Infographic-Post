...
export function SignInCard2({ googleEnabled = false }: Props) {
  ...
  return (
    <div className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-black">
      ...
      <div className="absolute left-1/2 top-0 h-[60vh] w-[120vh] -translate-x-1/2 transform rounded-b-[50%] bg-muted-foreground/20 blur-[80px]" />
      <motion.div
        className="absolute left-1/2 top-0 h-[60vh] w-[100vh] -translate-x-1/2 transform rounded-b-full bg-muted-foreground/20 blur-[60px]"
        animate={{ opacity: [0.15, 0.3, 0.15], scale: [0.98, 1.02, 0.98] }}
        transition={{ duration: 8, repeat: Infinity, repeatType: "mirror" }}
      />
      <motion.div
        className="absolute bottom-0 left-1/2 h-[90vh] w-[90vh] -translate-x-1/2 transform rounded-t-full bg-muted-foreground/20 blur-[60px]"
...
function BarsLogo() {
  return (
    <svg width={20} height={20} viewBox="0 0 24 24" aria-hidden>
      <rect x={3} y={14} width={4} height={7} rx={1} fill="#55534E" />
      <rect x={10} y={9} width={4} height={12} rx={1} fill="#F1EFEA" />
      <rect x={17} y={4} width={4} height={17} rx={1} fill="#FF4D00" />
    </svg>
  );
}
